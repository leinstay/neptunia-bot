...
// every hop), the header set, the content-type and size rules (declared and
// mid-stream), timeouts, HTTP and network failures, text/plain passthrough,
// HTML to text, charsets, and logging that carries host/path only. fetch and
// DNS lookup are fakes; nothing touches the network.
...
/** fetchImpl answering from `routes` (url -> response | Error | function), recording calls. */
...
test('fetchText: a scheme other than http/https fails with scheme, no lookup and no request', async () => {
  ...
  const fetcher = createPageFetcher({ fetchImpl, lookup: dns.lookup });
...
test('fetchText: localhost and IP literals fail with private, no request', async () => {
  const { calls, fetchImpl } = fakeFetch({});
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup });
...
test('fetchText: one private address among public ones is enough to refuse', async () => {
  const { calls, fetchImpl } = fakeFetch({});
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup({ 'mixed.example': [PUBLIC_V4, '::ffff:10.1.2.3'] }).lookup });
...
test('fetchText: more redirects than maxRedirects fail with redirects', async () => {
  ...
  const { result: byDefault } = await run(createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup }), 'https://r.example/0');
  assert.deepEqual(byDefault, { ok: false, reason: 'redirects' });
});

test('fetchText: sends the fixed header set with a GET', async () => {
  const { calls, fetchImpl } = fakeFetch({ 'https://h.example/': html('<p>x</p>') });
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup });
...
test('fetchText: a declared content-length over maxBytes fails with size before reading the body', async () => {
  ...
  const { fetchImpl } = fakeFetch({ 'https://s.example/': response });
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup });
...
test('fetchText: a body growing past maxBytes mid-stream fails with size and stops pulling', async () => {
  ...
  const { calls, fetchImpl } = fakeFetch({ 'https://s.example/big': response });
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup });
...
test('fetchText: a DNS lookup hanging past timeoutMs fails with timeout', async () => {
  const { calls, fetchImpl } = fakeFetch({});
  const fetcher = createPageFetcher({ fetchImpl, lookup: () => new Promise(() => {}) });
...
test('fetchText: a body stalling past timeoutMs fails with timeout', async () => {
  const fetchImpl = async (url, { signal }) => fakeResponse({
...
test('fetchText: a thrown fetch, a failed or empty lookup and a missing response fail with network', async () => {
  const { fetchImpl } = fakeFetch({ 'https://n.example/': new TypeError('fetch failed'), 'https://null.example/': null });
...
test('fetchText: HTML becomes readable text with its title, cut at maxChars', async () => {
  ...
  const { fetchImpl } = fakeFetch({ 'https://w.example/story': html(page) });
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup });
...
test('fetchText: a success logs nothing', async () => {
  const { fetchImpl } = fakeFetch({ 'https://ok.example/': html('<p>fine</p>') });
  const fetcher = createPageFetcher({ fetchImpl, lookup: fakeLookup().lookup });
