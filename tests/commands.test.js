...
test('buildCommandTree: top-level leaves (status, ping, reload, pause, resume, interject, initiate, set, unset)', () => {
  ...
  assert.deepEqual(pingRole.choices.map((c) => c.value), ['talk', 'analyzer', 'classifier.text', 'classifier.media', 'classifier.video', 'classifier']);
...
test('buildCommandTree: model group (show/set) role choices are talk/analyzer/classifier.text/classifier.media/classifier.video', () => {
  const [command] = buildCommandTree('nep');
  ...
  assert.deepEqual(role.choices.map((c) => c.value), ['talk', 'analyzer', 'classifier.text', 'classifier.media', 'classifier.video']);
});
