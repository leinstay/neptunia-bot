...
| `rewatch.md` | да | Классификатор: нужно ли персонажу пересмотреть видео или повторить загрузку незагрузившегося (`features.videoRewatch`). Получает список недавних видео с их статусом и новое сообщение. Выход: ОДНА строка: `<itemId> \| <question>`, `<itemId> \| retry` или `none` | `{{name}}` |
...
повторить загрузку незагрузившегося. Кандидаты — просмотренные видео и видео с ошибкой (последние — только пока у хода
осталась попытка `media.video.maxPerTurn`). Классификатору предлагается не более
`media.video.rewatch.maxCandidates` (по умолчанию 6) видео, от новейшего к старейшему. Код отправляет `rewatch.md`
как системный промпт на роли модели `rewatch` (`media.video.rewatch.model`, по умолчанию `mention.followUpModel`,
по умолчанию медиа-модель) с пользовательским сообщением, содержащим два блока:

```
<videos>
<itemId> | <name> | <status> | <начало описания>
...
</videos>
<candidate>
...
Каждая строка `<videos>` содержит четыре столбца через `|`: id элемента, название видео, статус (`watched` или
`not loaded`) и первые 200 символов описания (пусто для незагрузившихся видео). Видео перечислены от новейшего
сообщения к старейшему; названия и описания схлопнуты по пробелам в одну строку. Текст триггера обрезан до
`context.maxMessageChars`. Выход: ОДНА строка:

- `<itemId> | <question>` — сообщение спрашивает о просмотренном видео и требует деталь, не покрытую описанием.
- `<itemId> | retry` — сообщение о незагрузившемся видео и просит попробовать снова или спрашивает о его содержимом.
