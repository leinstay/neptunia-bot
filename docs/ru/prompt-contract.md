...
`followUp` (`mention.followUpModel`, по умолчанию `anthropic/claude-sonnet-4.6`). Выход: ОДНА строка: `yes`, когда кандидат адресован
...
как системный промпт на роли модели follow-up (`mention.followUpModel`, по умолчанию `anthropic/claude-sonnet-4.6`) с
