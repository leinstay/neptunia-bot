...
| `/nep model show` | Show active models for each role (`talk`, `analyzer`, `media`, `followup`, `video`) |
| `/nep model set <role> <id>` | Set the model for a role (`talk`, `analyzer`, `media`, `followup`, `video`) |
