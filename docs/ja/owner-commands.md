...
| `/nep ping [role]` | 一つまたはすべてのモデルロール（`talk`、`analyzer`、`media`、`followup`、`video`）にミニマルリクエストを送信し、モデル、レイテンシ、プロバイダー、トークン数またはエラーを報告。`video` ロールでは YouTube の到達性（このホストでどの再生時間ソースが動作するか）も報告。`llm.maxRequestsPerDay` にカウントされず、一時停止中やウォームアップ中でも動作 |
