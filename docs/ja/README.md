...
| `lookup.md` | いいえ | 分類器: 質問にウェブ検索が必要か |
| `read-link.md` | いいえ | フェッチしたページを 1 段落に要約 |
| `search-summary.md` | いいえ | 検索結果をソース付きの 1 つのノートに要約 |
...
ペルソナはメンション、リプライ、名前トリガーに応答しますが、時には無視することもあります。ランダムな間隔で会話に割り込み、静かなチャンネルで話題を切り出します。応答後、そのチャンネルのフォローアップメッセージを分類器で追跡します。サーバー全体で一度に一つのリプライを書きます。別のチャンネルからのピングは保留され、順番に応答されます。

メモリアナライザーは十分なメッセージが蓄積した時点で別途実行されます。メンバーごとのプロファイル（関心、詳細、エイリアス、エピソード、態度）、サーバー全体の傾向と内輪ネタ、イベントやストーリーのロアブックを構築します。プロファイルは差分更新されます。保存された事実が再要約されることはありません。ペルソナはチャットで人々が互いをどう呼ぶかも学び、名前やエイリアスで言及されたメンバーを認識します。

パイプラインの詳細、アナライザー、プロファイル、エピソード、ロアブック、メモリに関するオーナーコマンドについては[メッセージとメモリ](messages-and-memory.md)を参照してください。
...
`data/` にはメンバーごとのプロファイル、関係スコア、チャンネルの観察、サーバーパターン、キャッシュされたメディア説明文とウェブ抜粋が保存されます。あなたのマシン上に留まり、gitignore 対象で、LLM にはコンテキストとしてのみ送信されます。アナライザーはセンシティブな詳細を保存しないよう指示されています。`/nep memory forget` でプロファイルを完全に削除できます。
