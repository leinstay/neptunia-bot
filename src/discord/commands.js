        ...
        },
        {
          ...
          options: [
            ...
            { type: SUBCOMMAND, name: 'run', description: 'Start or resume the whole run: channels, then people, then the server.' },
            {
              type: SUBCOMMAND,
              name: 'user',
              description: '(Re)profile one member now.',
              options: [{ type: USER, name: 'user', description: 'Member to (re)profile.', required: true }],
            },
            { type: SUBCOMMAND, name: 'users', description: '(Re)profile every qualifying member now, in the background.' },
            {
              type: SUBCOMMAND,
              name: 'channel',
              description: '(Re)describe one channel now.',
              options: [
                {
                  type: CHANNEL,
                  name: 'channel',
                  description: 'Channel to (re)describe.',
                  required: true,
                  channel_types: [GUILD_TEXT],
                },
              ],
            },
            { type: SUBCOMMAND, name: 'channels', description: '(Re)describe every readable channel now, in the background.' },
...
const OPTION_MAPPERS = {
  ...
  'bootstrap.people': () => ({}),
  'bootstrap.run': () => ({}),
  'bootstrap.user': (options) => ({ userId: options.getUser('user', true).id }),
  'bootstrap.users': () => ({}),
  'bootstrap.channel': (options) => ({ channelId: options.getChannel('channel', true).id }),
  'bootstrap.channels': () => ({}),
  'bootstrap.server': () => ({}),
  'bootstrap.status': () => ({}),
  'bootstrap.reset': () => ({}),
