...
**English only.** Code, comments, commit messages, documentation, prompt defaults — everything in the repository is English. A deployment's `prompts.local/` can be in any language, but it is never committed.
